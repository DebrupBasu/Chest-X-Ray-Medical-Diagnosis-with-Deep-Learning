# -*- coding: utf-8 -*-
"""Chest X-Ray Classification using Densenet121.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1k_l8kLErcImMsJVnUDI-AW-Yfu0dsrmo
...
train_df = pd.read_csv("/content/drive/My Drive/Colab Notebooks/nih/train-small.csv")
valid_df = pd.read_csv("/content/drive/My Drive/Colab Notebooks/nih/valid-small.csv")

test_df = pd.read_csv("/content/drive/My Drive/Colab Notebooks/nih/test.csv")
...
IMAGE_DIR = "/content/drive/My Drive/Colab Notebooks/nih/images-small"
...
1. We can use `ModelCheckpoint` callback to monitor your model's `val_loss` metric and keep a snapshot of our model at the point. 
2. We can use the `TensorBoard` to use the Tensorflow Tensorboard utility to monitor our runs in real-time. 
3. We can use the `ReduceLROnPlateau` to slowly decay the learning rate for our model as it stops getting better on a metric such as `val_loss` to fine-tune the model in the final steps of training.
4. We can use the `EarlyStopping` callback to stop the training job when our model stops getting better in it's validation loss. We can set a `patience` value which is the number of epochs the model does not improve after which the training is terminated. This callback can also conveniently restore the weights for the best metric at the end of training to our model.
...
"""We can compare the performance to the AUCs reported in the original ChexNeXt paper in the table below:
...
For details about the best performing methods and their performance on this dataset, we read the following papers:
...
We will use a [GradCAM's](https://arxiv.org/abs/1610.02391) technique to produce a heatmap highlighting the important regions in the image for predicting the pathological condition. 
- This is done by extracting the gradients of each predicted class, flowing into our model's final convolutional layer. Look at the `util.compute_gradcam` which has been provided in `util.py` to see how this is done with the Keras framework. 
